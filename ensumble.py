...
from multiprocessing_env import train_mp, test_mp
import json
...
class MagicAgent:
    def __init__(self, lookback=50, lr=0.00005, epochs=1, optimizer=Adam, batch_size=32, model="", depth=0, comment=""):
        self.lookback_window_size = lookback
        self.model = model
        self.comment = comment
        self.depth = depth
        self.action_space = np.array([0, 1, 2])
        self.log_name = datetime.now().strftime("%Y_%m_%d_%H_%M")+"_Crypto_trader"
        self.state_size = (lookback, 5 + depth) # 5 standard OHCL information + market and indicators
        self.lr = lr
        self.epochs = epochs
...
def test_agent(test_df, test_df_nomalized, visualize=True, test_episodes=10, folder="", name="", comment="", Show_reward=False, Show_indicators=False):
    ...
    agent = MagicAgent(lookback=params["lookback window size"], optimizer=Adam, depth=params["depth"], model=params["model"])
...
if __name__ == "__main__":            
    ...
    df = add_indicators(df) # insert indicators to df 2021_02_21_17_54_Crypto_trader
    ...
    #train_mp(CustomEnv, agent, train_df, train_df_nomalized, num_worker = 32, training_batch_size=500, visualize=False, EPISODES=200000)
    ...
    test_mp(CustomEnv, MagicAgent, test_df, test_df_nomalized, num_worker = 16, visualize=True, test_episodes=1000, folder="2021_02_21_17_54_Crypto_trader", name="3263.63_Crypto_trader", comment="3 months")
